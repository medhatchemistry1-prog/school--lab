...
// غلاف طباعة موحّد لكل التقارير: يفرض تنسيق A4، رأس وذيل ثابتين،
// ومنع تقطيع صفوف الجدول بين الصفحات — يُستخدم مرة واحدة فوق أي تقرير مطبوع
...
// رأس رسمي موحّد (شعار + جهة الإصدار + عنوان التقرير + بيانات جانبية) تستخدمه كل التقارير الأربعة
function ReportLetterhead({
  ...
  return (
    <div className={`relative flex items-center justify-between gap-4 border-b-[3px] ${accentClass} pb-4 mb-6`}>
      <div className="text-right text-[10px] font-bold text-slate-600 leading-relaxed min-w-[150px]">
      ...
      <div className="flex flex-col items-center gap-0.5 flex-1">
        <AtomLogo />
        <h2 className={`text-lg font-black ${titleColorClass} text-center -mt-2`}>{title}</h2>
        {subtitle && <p className="text-[11px] text-slate-500 font-semibold text-center">{subtitle}</p>}
        <p className="text-[9px] font-mono text-slate-400 mt-0.5">كود التقرير: {reportCode}</p>
      </div>

      <div className="text-left text-[10px] font-bold text-slate-600 leading-relaxed min-w-[150px]">
...
// ذيل رسمي موحّد: طابع زمني، توقيع أمين المختبر، ورقم صفحة — يظهر أسفل كل تقرير مطبوع
function ReportFooter({ recordCount }: { recordCount: number }) {
  return (
    <div className="mt-8 pt-3 border-t border-slate-300 space-y-3">
      <div className="grid grid-cols-2 gap-8 pt-4 text-center text-[11px]">
        <div>
          <p className="font-bold text-slate-900 mb-8">توقيع أمين المختبر</p>
          <p className="text-slate-400">..............................</p>
        </div>
        <div>
          <p className="font-bold text-slate-900 mb-8">اعتماد إدارة المدرسة</p>
          <p className="text-slate-400">..............................</p>
        </div>
      </div>
...
export default function Home() {
  ...
  const refreshRequestsFromDb = async () => {
    try {
      const res = await fetch('/api/requests');
      const data = await res.json();
      if (data.success) setPrepRequests(data.requests || []);
    } catch (err) {
      console.error(err);
    }
  };

  // يحدّث القسم المطلوب من قاعدة البيانات مباشرة قبل فتح نافذة الطباعة،
  // حتى تعكس كل تقارير PDF أحدث بيانات مخزّنة فعلياً في Neon
        ...
        <div className="print-report report-request fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4 overflow-y-auto print:static print:flex-none print:items-start print:justify-start print:bg-transparent print:overflow-visible">
          ...
          >
            ...
            <div className="relative rounded-xl border-2 border-slate-800 bg-slate-50 p-4 print:bg-white print:border-2 print:border-slate-900">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex flex-col items-center gap-0.5">
                  <AtomLogo />
                  <h2 className="text-lg font-black text-slate-900 text-center -mt-2">استمارة تحضير وصرف تجربة مخبرية</h2>
                  <p className="text-[10px] font-mono font-bold text-slate-500 mt-0.5">كود الاستمارة: {selectedRequestForPrint.id}</p>
                </div>
            ...
            <p className="text-[9px] text-slate-400 text-center pt-2 border-t border-slate-200">
              تم إصدار هذه الاستمارة آلياً من قاعدة البيانات السحابية (Neon) بتاريخ {new Date().toLocaleString("ar-EG")}
            </p>
          </div>
        </div>
      )}
