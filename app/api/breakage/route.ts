...
// جلب جميع سجلات الكسر من قاعدة البيانات السحابية
...
// تسجيل كسر جديد وحفظه في قاعدة البيانات
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ success: true, message: "تم حفظ سجل الكسر في قاعدة البيانات بنجاح!" });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}

// حذف سجل كسر (عند إلغاء الخصم)
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({ success: true, message: "تم حذف سجل الكسر من قاعدة البيانات!" });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}